...
| Mission dashboard | **A1 is green and R1 is in progress.** The production HTML host loads a tested React shell; the 19 browser-facing schemas and polarity fixtures now specify its bootstrap, reads, reduced state, ordered stream, mutations, and replay inputs. Scenario/private-control schemas and strict service twins still block generated browser contracts and the remaining map, sources, controls, and runtime. Frontend coverage is independently inventory-adjudicated, while the unchanged 64 Playwright cases remain fixture acceptance rather than production end-to-end evidence ([FRONTEND_BUILD.md](docs/FRONTEND_BUILD.md)) |
