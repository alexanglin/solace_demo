...
| Mission dashboard | **A1 is green and R1 is in progress.** The production HTML host loads a tested React shell; 19 browser-facing schemas plus two scenario-file and eight private-control schemas now have manifest-owned polarity fixtures. Only strict service-owned Pydantic twins and HTTP/OpenAPI expectations still block A2's generated browser contracts; the remaining map, sources, controls, and runtime are not implemented. Frontend coverage is independently inventory-adjudicated, while the unchanged 64 Playwright cases remain fixture acceptance rather than production end-to-end evidence ([FRONTEND_BUILD.md](docs/FRONTEND_BUILD.md)) |
