...
| Durable mission store | **Schema and unit of work live.** The first Alembic revision is applied to a PostgreSQL 18.6 database the run creates and drops, both declared constraints are enforced, and above it a session, a bounded transaction, and the append-only audit log now run on a cluster: the server-side bounds and isolation level read back from a session, and two appenders for one mission ordered by the row lock the first holds ([ADR-0088](docs/adr/0088-order-the-mission-timeline-by-a-per-mission-audit-ordinal.md), [ADR-0089](docs/adr/0089-state-read-committed-rather-than-inherit-it.md), [`durable-store-first-run.md`](release-evidence/phase-3/durable-store-first-run.md)). The approval, idempotency, outbox, and ledger repositories, and the operator's own database, remain untouched |
