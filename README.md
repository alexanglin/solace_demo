...
that evidence exists, this repository does not claim an end-to-end operational demonstration.
...
| Durable mission store | **Implemented behind complete Alembic and SQLAlchemy boundaries; live upgrade qualification remains.** Nine append-only Alembic revisions own every application table and constraint, and async SQLAlchemy 2.x Core repositories and service-specific units of work own all runtime access. Deterministic tests cover revision-by-revision upgrade/downgrade, transactions, races, conflict digests, bounds, recovery, and ordered export. The original four-revision PostgreSQL live evidence remains valid, but revisions five through nine still require the authorized created-and-dropped probe-database and shared-stack migration run before release ([ADR-0003](docs/adr/0003-postgres-durable-mission-store.md), [ADR-0146](docs/adr/0146-define-durable-application-processing.md), [ADR-0151](docs/adr/0151-require-migrated-sqlalchemy-durable-tables.md)) |
| Mission dashboard | **Implemented and Compose-wired; packaged live acceptance remains.** The FastAPI service runs behind Caddy over a private Unix socket, enforces exact Host/Origin/bearer and bounded-body rules, consumes validated broker events, recovers persisted projection state, and exposes bounded SSE plus idempotent command and proposal-decision mutations. The React runtime validates bootstrap/SSE/mutation documents and renders live, degraded-live, and replay state with accessible approval controls. Deterministic coverage and fixture Playwright acceptance are green; the shared-stack packaged-browser and reconnect observations are still pending ([FRONTEND_BUILD.md](docs/FRONTEND_BUILD.md)) |
