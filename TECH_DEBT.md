...
[ADR-0019](docs/adr/0019-fail-closed-quality-gates.md) had left open by design. The current inventory
classifies only `services/evidence_service` as `SCAFFOLD`; every other workspace member has executable
production behavior and is measured rather than exempted. A member with nothing to measure proves
nothing either way
...
| The local models are spike inputs, not a measured choice | `ollama_chat/mistral:7b` replaced `ollama_chat/qwen3:4b` on a measurement rather than a spike: the same sector-planning question took 104.9 s and 4,623 evaluated tokens on qwen3 against 9.4 s and 566 on mistral, because qwen3 reports the `thinking` capability and spends it. `ollama_chat/qwen3:4b` had been selected for tool capability at 2.50 GB. The coordinator's move to `ollama_chat/llama3:8b` ([ADR-0198](docs/adr/0198-give-the-coordinator-a-model-and-a-tool-surface-that-answer.md)) was reversed the next day: that model reports no `tools` capability, so every completion under it failed ([ADR-0200](docs/adr/0200-give-the-coordinator-a-tool-capable-model.md)). One model remains locked, chosen on a spike rather than an evaluation, and the `general` and `planning` roles are still an open question in [docs/adr/README.md](docs/adr/README.md) | The Phase 0 evaluation measures capability per dollar and Phase 4 pins the three models |
| Nothing refuses a model that cannot do what its agent needs | ADR-0200 requires a tool-bearing agent's model to report the `tools` capability, and that requirement is prose. The offline validator proves only that a configured model is locked with a well-formed digest, and readiness compares digests through `GET /api/tags`, which does not carry capabilities. Reading them needs `GET /api/show` and a `capabilities` field in the lock, whose key set the validator pins exactly | The lock records each model's capabilities and the validator refuses an agent whose model lacks one it needs |
...
| ~~Command intake settles after publisher confirmation rather than after a durable commit~~ **Implemented 2026-08-27; live qualification pending:** command-gateway ingress now commits idempotency, approval consumption, audit, and command-outbox staging before settlement. Fleet command effects and receipts commit before settlement, and duplicate delivery returns the prior result. Deterministic crash and replay tests cover the transaction boundaries; the shared-stack broker-restart proof remains release evidence rather than an implementation substitute ([ADR-0146](docs/adr/0146-define-durable-application-processing.md), [ADR-0189](docs/adr/0189-reconcile-dashboard-runtime-with-the-solace-data-plane.md)) | -- | Cleared in code; qualify live before release |
| ~~The fleet simulator has no process entry point, so its Compose service is still a shell~~ **Cleared 2026-08-27:** fleet and scenario have production entrypoints, authenticated private start/status/cancel control, an exact twenty-member scenario projection, interruptible pacing, direct telemetry, critical-event outbox publication, and durable per-drone command consumers. Scenario remains brokerless; dashboard owns committed mission-event publication ([ADR-0158](docs/adr/0158-keep-scenario-control-brokerless.md), [ADR-0189](docs/adr/0189-reconcile-dashboard-runtime-with-the-solace-data-plane.md)) | -- | Cleared |
...
| The scenario catalog's projection constraints (the 23/20/3/20 workload counts, the 512 KiB bound) are evaluated on request, not at startup, so `/readyz` can be 200 while `GET /internal/v1/scenarios` answers `INTERNAL_FAILURE`; projecting at startup would refuse the golden minimal definition the established catalog tests load | `services/scenario_service/src/aerial_rescue_scenario_service/catalog.py` | The catalog tests are rewritten against a projectable fixture and the projection joins readiness, with explicit permission for the expectation change |
| A private route that does not exist, or a method the route does not accept, is answered by the framework (plain 404/405, no closed refusal document, no `Cache-Control: no-store`) before Host and bearer admission, on both the scenario and fleet private listeners | `services/scenario_service/src/aerial_rescue_scenario_service/http_runtime.py`, `services/fleet_simulator/src/aerial_rescue_fleet_simulator/control_plane/runtime.py` | A decision fixes the closed shape for unmatched routes and both runtimes register the handler test-first |
| A fleet start reply whose identities do not match the request is refused `FLEET_UNAVAILABLE` without binding the run, so an identical retry sends a second fleet start; `docs/CONTRACTS.md` says a caller never automatically repeats start | `services/scenario_service/src/aerial_rescue_scenario_service/control.py` | The mismatched reply binds the run as `FLEET_UNAVAILABLE` for the epoch or a decision names the retry semantics |
| The catalog document is re-encoded on every request and its 512 KiB check runs after the full encoding (the projection itself is cached per epoch) | `services/scenario_service/src/aerial_rescue_scenario_service/http_runtime.py` | The encoded bytes are cached with the projection once a measurement shows the encoding on the request path matters |
| Every service image except PostgreSQL and Caddy runs as a non-root user (`aerial-rescue/application` is `USER 10001:10001`, the pinned Event Management Agent is `emauser`), so their mode-0600 mounted secrets are unreadable on a Linux host; [ADR-0203](docs/adr/0203-make-the-broker-s-mounted-secrets-readable-by-the-pinned-image-s-user.md) fixed and proved only the broker's two files, because the blocking job starts only the broker and PostgreSQL | `scripts/broker-secrets.sh`, `deploy/compose.yaml` | Each remaining service's mounted secrets are given a mode its image's user can read, with a live Linux run of the composed stack as the evidence |
