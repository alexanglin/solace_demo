...
| --- | --- | --- |
| default | `broker` (PubSub+ Standard 10.26.0), `postgres` (PostgreSQL 18.6), and `agent-mesh`, built on the official `solace/solace-agent-mesh:1.28.7` image with the two pinned Event Mesh wheels installed by hash | Runnable. The mesh carries seven apps: the Orchestrator, the MissionCoordinator agent, the SectorPlanner and EvidenceFusion agents, the MissionResponse workflow, the HTTP/SSE Web UI, and the Event Mesh Gateway. The two node agents serve the workflow and save their own output artifacts; the MissionCoordinator serves the Event Mesh Gateway and does not ([ADR-0218](adr/0218-make-a-workflow-node-s-agent-save-its-own-output-artifact.md)). It joined the default profile when `agent-mesh/configs/` landed, which is the condition [ADR-0044](adr/0044-docker-compose-runtime-with-official-agent-mesh-image.md) set and [ADR-0102](adr/0102-start-the-agent-mesh-with-the-default-profile.md) executes. It still needs a local model, so `just up` refuses to start it unless the locked model is served |
