...
| --- | --- | --- | --- |
| B01 | Re-POST an already-consumed approval with the same idempotency key | `409`, hard denial, audited as a bypass attempt. Explicitly **not** an idempotent success ([ADR-0006](../adr/0006-proposal-bound-single-use-approvals.md)) | **durable half proven live** (`tests/integration/test_durable_store_live.py`): the approval row denies the repeat with the protocol's own `ALREADY_CONSUMED`, and the claim on the same key denies it independently ([ADR-0091](../adr/0091-consume-an-approval-under-its-own-row-lock.md), [ADR-0092](../adr/0092-claim-an-idempotency-key-with-one-conflicting-insert.md)); gateway, audit, and HTTP halves to build |
| B02 | Re-POST a consumed approval with a *fresh* idempotency key | `409`, hard denial, audited | **durable half proven live** (`tests/integration/test_durable_store_live.py`): the approval row is the authority, so a fresh key does not evade it. ADR-0092 records this as the reason the claim is a second line rather than the line; gateway, audit, and HTTP halves to build |
| B03 | Reuse one idempotency key with a different request body | `409`, refused on body-hash mismatch | **durable half proven live** (`tests/integration/test_durable_store_live.py`): the stored digest is compared and a difference is `BODY_MISMATCH`, which is a refusal rather than a repeat; gateway and HTTP halves to build |
| B04 | Redeliver the escalation command with the original command ID after it completed | Prior result returned; no second dispatch | **durable half proven live** (`tests/integration/test_durable_store_live.py`): the claim returns the persisted result and the decision comes from `packages/domain`; gateway and HTTP halves to build, including the in-flight window ADR-0092 refuses rather than answers |
| B05 | Redeliver the escalation command with a new command ID but the same approval | Refused; the approval is spent | **durable half proven live** (`tests/integration/test_durable_store_live.py`): consumption is refused by the approval row whatever identifier the redelivery carries; gateway and HTTP halves to build |
...
| # | Attempt | Required outcome | Status |
| --- | --- | --- | --- |
| B07 | Two concurrent consumptions of the same approval | Exactly one succeeds; the other is denied. Asserted under real concurrency, not sequentially | **durable half proven live, under real concurrency** (`tests/integration/test_durable_store_live.py`): two consumers on one PostgreSQL row commit once and deny once, with the second observed still *waiting* on the row lock rather than merely finishing later, and refused by the domain's own `ALREADY_CONSUMED` ([ADR-0091](../adr/0091-consume-an-approval-under-its-own-row-lock.md)); gateway and HTTP halves to build |
