...
| Agent Mesh forced-stop grace | 45 s, covering the pinned Connector's own stop and cleanup and the owned settle window that follows it | `stop_grace_period` on the `agent-mesh` service, held against `THREAD_SETTLE_SECONDS` by a deployment test ([ADR-0199](adr/0199-terminate-the-owned-agent-mesh-entrypoint.md)) |
