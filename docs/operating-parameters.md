...
| Endpoints the reference fleet needs | 83 broker-managed queues: 40 application primaries (20 family and 20 per-drone), 40 paired application DMQs, and 3 upstream-template DMQs; up to 8 bounded upstream temporary primaries coexist (four, two, and one at steady state plus the coordinator's reply queue, ADR-0196). Twenty per-drone queues rather than twenty-three, because a declared-only member receives none ([ADR-0118](adr/0118-provision-command-queues-only-for-executable-members.md)) | derived from the current grants and asserted by `test_the_executable_reference_fleet_reserves_eighty_three_queues_and_830_megabytes`; `test_the_reference_fleet_reserves_eighty_nine_queues_and_890_megabytes` pins the same arithmetic for a twenty-three-member roster. Measured live on 2026-08-31: 89 durable queues fell to 83 once `just retire-stale-queues` removed the six the declared-only members still held. The message VPN's measured ceilings are 1,000 endpoints and 1,500 MB of spool |
