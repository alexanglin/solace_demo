...
| Orchestration model for the Orchestrator and the workflow | `ollama_chat/mistral:7b`, 4.37 GB on disk at 7.2B parameters, reporting `completion` and `tools` | `agent-mesh/model-lock.toml`. Provisional: the roles are pinned by the Phase 4 model selection |
| Model for the coordinator's structured answer | `ollama_chat/mistral:7b`, 4.37 GB, reporting `completion` and `tools` and no `thinking` | `agent-mesh/model-lock.toml` and the `model` in `agent-mesh/configs/mission-coordinator.yaml` |
