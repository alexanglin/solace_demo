# ADR-0120: Bind proposals to the complete source event

- **Status:** Accepted
- **Date:** 2026-08-25
- **Deciders:** Alex Anglin
- **Supersedes:** none; defines the `sourceEventDigest` preimage left implicit by ADR-0116

## Context

[ADR-0116](0116-close-the-application-data-plane-wire-documents.md) requires every candidate Agent
...
decorative precisely where ADR-0116 intends a provenance check.
...
- **Leave the 64-character value opaque.** Rejected because a value with no defined preimage cannot be
  recomputed, compared across services, or serve the digest-mismatch refusal ADR-0116 names.
