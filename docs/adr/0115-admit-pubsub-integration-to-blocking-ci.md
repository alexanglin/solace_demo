# ADR-0115: Admit PubSub+ integration to blocking continuous integration
...
ADR-0114 adds a second missing claim. The application data plane must normalize an untrusted direct
...
| 7 | `tests/integration/test_application_data_plane_live.py` | ADR-0114's broker-inbox, PostgreSQL commit, application-outbox, normalization, evidence-decision, exact approval, command, and durable-receipt path |
