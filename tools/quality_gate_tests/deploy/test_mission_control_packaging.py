"""Structural acceptance tests for the closed mission-control Compose runtime."""
...
class MissionControlClosureTests(QualityGateTestCase):
    ...
    def test_the_private_control_secrets_are_distinct_and_least_privilege_mounted(self) -> None:
        # Arrange
        services = _services()
        ...
        scenario_users = {
            ...
            if "scenario-control-secret" in _secrets(service)
        }
        fleet_users = {
            name
            for name, service in services.items()
            if "fleet-control-secret" in _secrets(service)
        }

        # Assert
        self.assertIn("scenario-control-secret", declared)
        self.assertIn("fleet-control-secret", declared)
        self.assertEqual(
            "./secrets/scenario-control-secret",
            cast("Mapping[str, str]", declared["scenario-control-secret"])["file"],
        )
        self.assertEqual(
            "./secrets/fleet-control-secret",
            cast("Mapping[str, str]", declared["fleet-control-secret"])["file"],
        )
    ...
    def test_recorder_uses_its_mounted_broker_secret_and_active_readiness_lease(self) -> None:
        ...
        # Assert
        self.assertEqual(
            "/run/secrets/recorder-broker-password",
            recorder_environment.get("SOLACE_BROKER_PASSWORD_FILE"),
        )
        self.assertNotIn("SOLACE_RECORDER_PASSWORD", recorder_environment)
        self.assertIn("recorder-broker-password", _secrets(recorder))
    ...
    def test_mission_control_fleet_is_publication_only(self) -> None:
        # Arrange
        fleet = _services()["fleet-simulator"]
        recipe = _recipe(JUSTFILE_PATH.read_text(encoding="utf-8"), "mission-control-up")

        # Act
        environment = cast("Mapping[str, str]", fleet["environment"])

        # Assert
        self.assertEqual(
            "${AERIAL_RESCUE_FLEET_COMMAND_INTAKE_MODE:-enabled}",
            environment.get("FLEET_COMMAND_INTAKE_MODE"),
        )
        self.assertIn("AERIAL_RESCUE_FLEET_COMMAND_INTAKE_MODE=publication-only", recipe)
...
class MissionControlNetworkTests(QualityGateTestCase):
    ...
    def test_every_mission_control_network_is_internal_and_need_to_know(self) -> None:
        ...
        expected = {
            ...
            "migration": {"store"},
            "fleet-simulator": {"event-mesh", "scenario-fleet"},
            "scenario-service": {"event-mesh", "scenario-fleet", "dashboard-scenario"},
            "recorder": {"event-mesh", "store"},
            "dashboard-api": {"store", "dashboard-scenario"},
...
class MissionControlRecipeTests(QualityGateTestCase):
    ...
    def test_operator_recipes_select_only_reachable_service_sets(self) -> None:
        ...
        self.assertEqual(
            " ".join(MISSION_CONTROL_LONG_RUNNING_SERVICES),
        ...
        for service in SHARED_BASE_SERVICES:
            self.assertNotRegex(expected, rf"\b{service}\b")
