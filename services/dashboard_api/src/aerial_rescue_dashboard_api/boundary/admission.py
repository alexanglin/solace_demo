"""HTTP-boundary admission checks that precede routing and all request-body effects."""
...
from aerial_rescue_dashboard_api.boundary.responses import error_response

_SCENARIO_MUTATION_PREFIX = b"/api/v1/scenarios/"
_COMMAND_PATH_PARTS = 6
_DECISION_PATH_PARTS = 8
...
def _is_mutation(scope: Scope) -> bool:
    """Recognize exactly the four accepted state-changing path shapes."""
    method = scope.get("method")
    path_value = scope.get("path")
    if method != "POST" or not isinstance(path_value, str):
        return False
    path = path_value.encode()
    if path == b"/api/v1/scenarios/current/reset" or (
        path.startswith(_SCENARIO_MUTATION_PREFIX) and path.endswith(b"/start")
    ):
        return True
    parts = path.split(b"/")
    return (
        len(parts) == _COMMAND_PATH_PARTS
        and parts[:4] == [b"", b"api", b"v1", b"missions"]
        and parts[4] != b""
        and parts[5] == b"commands"
    ) or (
        len(parts) == _DECISION_PATH_PARTS
        and parts[:4] == [b"", b"api", b"v1", b"missions"]
        and parts[4] != b""
        and parts[5] == b"proposals"
        and parts[6] != b""
        and parts[7] == b"decisions"
    )
