"""FastAPI composition, raw boundary, route, and response tests."""
...
_FIXTURES = Path(__file__).parents[3] / "fixtures" / "golden" / "v1" / "dashboard"
...
def test_fastapi_route_inventory_is_exact_and_has_no_framework_admin_routes() -> None:
    # Arrange
    app = create_live_application(
        _settings(), _context(), _ready(RunMode.DEGRADED_LIVE), _LiveOperations()
    )
...
async def test_health_stays_live_while_mode_specific_readiness_tracks_dependency_loss() -> None:
    ...
    app = create_live_application(_settings(), _context(), readiness, operations)
...
async def test_raw_host_boundary_refuses_before_mutation_body_or_operation_effect() -> None:
    ...
    operations = _LiveOperations()
    app = create_live_application(
        _settings(), _context(), _ready(RunMode.DEGRADED_LIVE), operations
    )
...
async def test_mutation_before_lifespan_start_is_refused_without_opening_resources() -> None:
    ...
    app = create_live_application(
        _settings(),
        _context(),
        RuntimeReadiness(RunMode.DEGRADED_LIVE),
        operations,
    )
    transport = httpx.ASGITransport(app=app)
