"""FastAPI boundary, ordered ingress security, caching, and exact responses."""
...
class _Mutations:
    ...
    async def command(self, mutation: AuthorizedMutation) -> bytes:
        ...
        return canonical.canonical_bytes(canonical.decode(dashboard_fixture("command-response")))

    async def decide(self, mutation: AuthorizedMutation) -> bytes:
        self.calls.append(mutation)
        return canonical.canonical_bytes(
            canonical.decode(dashboard_fixture("proposal-decision-response"))
        )
...
class PublicReadTests(unittest.IsolatedAsyncioTestCase):
    async def test_live_readiness_and_lifespan_follow_broker_recovery_and_reverse_shutdown(
        self,
    ) -> None:
        # Arrange
        broker = _Broker()
        application, store, _, _ = _application(broker=broker)
        ...
        # Act
        before = await _request(application, "GET", "/api/v1/readiness?mode=degradedLive")
        ...
        # Assert
        self.assertEqual((503, 200), (before.status_code, recovered.status_code))
        self.assertEqual(
            ["startup", "activate:mission-test-0001", "shutdown"],
            broker.calls,
        )
...
class MutationSecurityTests(unittest.IsolatedAsyncioTestCase):
    async def test_live_mutation_routes_refuse_invalid_or_unbound_authority_before_staging(
        self,
    ) -> None:
        # Arrange
        application, store, _, _ = _application()
        command = canonical.canonical_bytes(
            canonical.decode(dashboard_fixture("operator-command-request"))
        )
        decision = canonical.canonical_bytes(
            canonical.decode(dashboard_fixture("proposal-decision-request"))
        )
        ...
        invalid_path = await _request(
            application,
            "POST",
            "/api/v1/missions/INVALID/commands",
            content=command,
            headers=_mutation_headers(),
        )
        no_run = await _request(
            application,
            "POST",
        ...
        store.current = CurrentRun(
            ...
            scenario_id="wilderness-missing-person",
            scenario_revision=1,
            mission_id="mission-synthetic-other",
            run_id="run-test-other",
            session_id=None,
        )
        wrong_mission = await _request(
            application,
            "POST",
            "/api/v1/missions/mission-synthetic-0001/commands",
            content=command,
        ...
        self.assertEqual(
            ...
            [
                ...
                for response in (invalid_path, no_run, wrong_mission, command_port, decision_port)
            ],
        )

    async def test_live_application_mutations_stage_exact_documents_and_replay_refuses_writes(
        self,
    ) -> None:
        # Arrange
        mutations = _Mutations()
        application, store, _, _ = _application(mutations=mutations)
        store.current = CurrentRun(
            mode=RunMode.DEGRADED_LIVE,
            scenario_id="wilderness-missing-person",
            scenario_revision=1,
        ...
        # Act
        command_response = await _request(
            application,
            "POST",
            "/api/v1/missions/mission-synthetic-0001/commands",
            content=command,
        ...
        decision_response = await _request(
            application,
            "POST",
            "/api/v1/missions/mission-synthetic-0001/proposals/proposal-synthetic-0001/decisions",
            content=decision,
        ...
        self.assertEqual(
            ...
            (
                ...
                replay_refusal.status_code,
            ),
        )
        self.assertEqual("REPLAY_READ_ONLY", replay_refusal.json()["errorCode"])
        self.assertEqual(2, len(mutations.calls))
        self.assertTrue(all(item.operator_id == "local-operator" for item in mutations.calls))
        ...
        self.assertEqual(
            {
                ...
                ("POST", "/api/v1/missions/{missionId}/commands"),
                (
                    "POST",
