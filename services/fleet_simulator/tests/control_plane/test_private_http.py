...
import httpx
import pytest
from aerial_rescue_fleet_simulator.control_plane.control import FleetControl
from aerial_rescue_fleet_simulator.control_plane.http import (
    CONTROL_PORT,
    FleetHttpConfig,
    create_app,
    serve,
...
_exchange: Final = asgi_exchange_for("http://fleet-simulator:8082")
...
class FleetHttpAdmissionTests(unittest.TestCase):
    ...
    def test_configuration_and_listener_are_strict_and_internal_only(self) -> None:
        ...
        with patch("aerial_rescue_fleet_simulator.control_plane.http.uvicorn.run") as run:
            serve(app)
